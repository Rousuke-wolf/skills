...
// 默认音色（文化页）；教学页可在请求体里传 voice 字段覆盖
...
const server = http.createServer((req, res) => {
  ...
  req.on("end", () => {
    let text, volume, rate, voice;
    try {
      ({ text, volume, rate, voice } = JSON.parse(body));
    } catch {
      res.writeHead(400); res.end("Bad request"); return;
    }

    if (!text?.trim()) { res.writeHead(400); res.end("Empty text"); return; }
...
server.listen(PORT, () => {
  ...
  console.log("   默认音色：" + DEFAULT_VOICE + "，模型：" + MODEL);
});